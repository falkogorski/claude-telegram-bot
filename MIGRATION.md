...
- **2026-07-24 (17)** — **1.9 Webhook UMGESCHALTET (mit Adam „Webhook los").** Polling → Webhook, Self-Signed direkt auf `159.195.195.82:8443`. Rote Auflagen technisch verifiziert: secret_token (falsches → HTTP 403), unerratbarer Pfad, ufw 8443 nur Telegram-Netze; Bot lauscht 0.0.0.0:8443, Zertifikat von Telegram akzeptiert. Env-Backup vor Umstellung. Rollback dokumentiert (BOT_MODE raus → Polling). Offen: Adams Live-Test-Nachricht (<2 s) → dann VERIFIZIERT.
...
- **Status:** 🔄 **UMGESCHALTET + technisch verifiziert (24.07.2026, mit Adam „Webhook los"):** Self-Signed direkt auf die IP `159.195.195.82:8443`. `getWebhookInfo`: URL gesetzt, `has_custom_certificate: true`, `pending: 0`, kein neuer Fehler (der `Connection refused` steht eingefroren auf dem Startmoment 14:08:52 = einmaliger Startup-Race). Lokaler Endpoint-Test: korrektes Secret → HTTP 200, **falsches Secret → 403** (secret_token greift). Bot lauscht `0.0.0.0:8443`. **Rote Auflagen erfüllt:** secret_token (64 Hex), unerratbarer Pfad (48 Hex), ufw 8443 nur für `149.154.160.0/20` + `91.108.4.0/22`. Cert/Key unter `/etc/claude-telegram-bot-webhook/` (claudebot-lesbar, Key 640). Env-Backup vor der Umstellung gesichert. **Rollback:** `BOT_MODE` (+ Webhook-Envs) in `/etc/claude-telegram-bot.env` entfernen → Neustart → Polling. **Offen: Adams Live-Test** (eine Nachricht kommt in < 2 s an) → dann VERIFIZIERT.
