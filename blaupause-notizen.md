...
- **Eine Verkettungs-Pruefung, die eine Zerlegung bekommt, wird unerreichbar — und ihre Luecke unsichtbar** · F-20 · **universell** — `WEITERE_VERKETTUNG` sollte alles fangen, was diese Datei nicht zerlegt. Als am 01.09. die Zerlegung an `;`/`|`/`||` dazukam, faengt die alles davon **vorher** — die Zeile wurde toter Code. **Und mit ihr wurde unsichtbar, was sie NIE gefangen hatte:** das freistehende `&`. `ls & curl boese.example` war frei, in jedem Modus, ohne Rueckfrage. **Tatsaechlich eingetretene Nebenwirkung beim Pruefen:** Von sechs notierten Pruefzeilen blieben zwei auch ohne die neue Schranke gruen — sie fielen ueber die Positivliste bzw. den Pfad. Waeren sie so stehengeblieben, haetten sie eine spaetere Entkernung gedeckt. **Lehre: Wenn eine neue Pruefung eine alte ueberfluessig macht, wird die alte nicht nur geloescht oder vergessen — es wird gemessen, was sie ABGEDECKT HAT und die neue nicht.** Eine tote Wache hinterlaesst nicht nur Leere, sondern den Eindruck, ihr Bereich sei bewacht.
- **Ein Zugangsname aus einem fremden Skript wird gemessen, nicht abgeschrieben** · U-6 / Befehlsblock · **universell** — Ich habe `claudebot@vps` in einen Befehlsblock UND in ein Skript geschrieben, weil `vps_backup.sh` einen Host `claudevps` nennt. Der Name existiert nicht; die echten Eintraege stehen in `~/.ssh/config` (`claudebot` und `claudevps`, beide auf dieselbe Adresse, verschiedene Nutzer). Adam hat es beim ersten Befehl gesehen. **Tatsaechlich eingetretene Nebenwirkung, die schwerer wiegt als der Tippfehler:** Im Skript waere derselbe Fehler NIE aufgefallen — Haelfte 1 quittierte mit der Sammelzeile *„Server nicht erreichbar ODER Ordner fehlt"*, und das las sich vor dem Umzug wie der Normalfall. **Ein unscharfer Grund macht einen echten Ausfall vom Normalfall ununterscheidbar.** Jetzt drei getrennte Lagen: Server antwortet nicht (FEHLER) · Ordner fehlt noch (uebersprungen) · Uebertragung gescheitert (FEHLER). **Lehre: Wer eine Sammelmeldung schreibt, entscheidet damit, welche Lagen er kuenftig nicht mehr unterscheiden kann** — und das faellt erst auf, wenn die falsche eintritt.
- **Eine Erinnerung braucht keinen Zeitgeber, sondern einen geteilten Ablauf** · N-1 · **universell** — Die naheliegende Bauform fuer „erinnere nach 15/30/45 Minuten" ist ein eigener Zeitgeber. Er waere falsch: Er bekaeme die Entscheidung nicht mit und schriebe weiter, nachdem Adam laengst gedrueckt hat — genau das, was er „sehr stoerend" nennt und was binnen einer Woche abgeschaltet wird. Richtig ist, **das Warten selbst zu zerlegen**: Nach jedem Abschnitt prueft die Schleife, ob entschieden wurde. Die Erinnerungen verstummen dann nicht, weil jemand sie stoppt, sondern **weil es sie in dem Moment nicht mehr gibt**. **Tatsaechlich eingetretene Nebenwirkung, die den Bau fast gekippt haette:** `asyncio.wait_for` bricht bei Zeitablauf ab, worauf es wartet — ohne `shield` waere das Future nach dem ersten Abschnitt tot gewesen und Adams Knopfdruck ins Leere gelaufen. Die Anfrage waere schlechter dran gewesen als mit einem einzigen langen Warten. **Lehre: Wer ein langes Warten in Abschnitte teilt, muss pruefen, ob das Gewartete die Teilung ueberlebt.**
- **Ein Pruefer in Sekundenbruchteilen findet Fehler, die in Minuten unsichtbar sind** · N-1 · **universell** — Der Pruefstand faehrt die Frist mit `0,4` statt `3600` Sekunden. Dabei fiel ein Fliesskomma-Rest auf: `0,4` minus viermal `0,1` ergibt `2.7e-17`, und das ist groesser als null — ein Abschnitt zuviel lief. Mit ganzen Sekunden waere es nie aufgefallen, weil die Einstellgroessen heute `int` sind. **Tatsaechlich eingetretene Nebenwirkung:** Derselbe Pruefstand fand zwei weitere eigene Fehler — einen werfenden Erinnerer, der die ganze Anfrage mitriss, und eine Pruefzeile, die ueber ihren **eigenen Erklaerkommentar** stolperte (er zitiert die alte Zahl, die sie verbietet). **Lehre: Ein Pruefer, der die Groessen verkleinert statt die Zeit zu verkuerzen, misst dieselbe Logik unter anderem Licht — und sieht dort, was im Normalmass verdeckt bleibt.**
- **Eine Wortliste mit Vollformen faengt die gebeugte Form nicht** · N-4 · **universell** — Die Umlaut-Liste fuehrte `aendern`, und die Pruefung vergleicht Teilzeichenfolgen. `aendert` enthaelt `aendern` **nicht** — genau die drei Woerter, die Adam bemaengelt hat, standen nicht drauf. **Der schwerere Teil des Befunds war aber nicht die Liste, sondern die Reichweite:** Der Pruefer lief ausschliesslich ueber einen Pfad (das Postfach) und sah den Text, an dem der Fehler auftrat, ueberhaupt nicht. **Tatsaechlich eingetretene Nebenwirkung:** Die Reparatur bestand nicht darin, einen Pruefer zu bauen, sondern die Meldungstexte aus dem Handler in **Modulkonstanten** zu ziehen — danach erreichte sie der VORHANDENE Pruefer, ohne neuen Waechter. **Lehre: Wenn ein Pruefer eine Stelle nicht sieht, ist oft die Stelle falsch gelegen, nicht der Pruefer zu schwach.**
- **Eine Zahl in einer Pruefzeile ist die Umsetzung der Zusage, nicht die Zusage** · N-2 · **universell** — Die Zeile „Fremdtext baut den Dialog nicht um" verlangte `len(kopf) == 2`. Die Zusage lautete: *Fremdtext erzeugt keine zusaetzliche Zeile.* Als der Dialog eine **legitime** Kopfzeile dazubekam, brach die Zahl, obwohl die Zusage unberuehrt war. **Tatsaechlich eingetretene Nebenwirkung:** Der Reflex war, die Zahl auf 3 zu setzen — dann waere sie beim naechsten Wachstum wieder gebrochen, und irgendwann haette jemand die Zeile geloescht. Gemessen wird jetzt der **Unterschied** zum selben Aufruf ohne Fremdtext. Das ist strenger als jede feste Zahl und ueberlebt Wachstum. **Lehre: Wenn eine Pruefzeile an einer legitimen Aenderung bricht, misst sie eine Groesse statt einer Zusage — dann wird sie umgestellt, nicht nachgezogen.**
- **Ein „Pruefung folgt" ohne Termin ist ein „nie"** · N-5, Engywucks eigene Zeile · **universell** — Claudias Auftrag vom 31.08. stand zwei Tage bei „Pruefung folgt", und Adam hat genau diese Klasse am selben Tag geruegt. Der Punkt, der darin lag (die Freigabe-Erinnerungen), fiel ihm am 02.09. abends als fehlend auf — zwei Tage spaeter, im Betrieb. **Lehre: Ein Vermerk, der eine Pruefung ankuendigt, braucht ein Datum; sonst ist er eine Ablage, in der etwas verschwindet, und sieht dabei aus wie Sorgfalt.**
- **Ein Ausschluss ueber einen NAMEN schuetzt, was so heisst — nicht, was so IST** · Z-2 · **universell** — Das Backup sollte `~/workspace` als Ganzes sichern, ausgeschlossen nur Reproduzierbares. Erster Anlauf: `--exclude='.venv'`. Der Trockenlauf mass trotzdem **1,1 GB und 14.343 Dateien** — unter `~/workspace/.nemo-test/` lag eine Python-Umgebung, die schlicht anders heisst. **Tatsaechlich eingetretene Nebenwirkung:** Der Wechsel auf `site-packages` (eine Eigenschaft, die JEDE Python-Umgebung hat) brachte **393 Dateien und 174 MB** — Faktor 36, mit einer geaenderten Zeichenkette. **Lehre: Auch beim AUSSCHLIESSEN gilt die Mengen-Regel.** Ich hatte sie beim Einschliessen richtig angewandt (Baeume statt Liste) und beim Ausschliessen im selben Atemzug gebrochen.
- **Eine Zahl, die offensichtlich falsch ist, entwertet die ganze Meldung** · Z-1b · **universell** — Die Alterszeile des Tageschecks rechnete mit `find -printf` und `stat -c %Y`; beides ist GNU-eigen. Auf dem Mac scheitern beide **still**, der Rueckfall lieferte 0, und die Meldung sagte *seit 20698 Tagen*. **Tatsaechlich eingetretene Nebenwirkung, die schwerer wiegt als der Fehler selbst:** Waere die Zahl nur *knapp* falsch gewesen — vier statt drei Tage —, haette sie niemand geprueft, und die Meldung waere dauerhaft ungenau geblieben. Behoben durch **Staffelung statt Rechnung** (`find -mtime +30/+14/+7/+3/+1`, ueberall vorhanden); die Aussage heisst jetzt ehrlich *seit mindestens N Tagen*. **Lehre: Wo eine genaue Zahl portabel schwierig ist, ist eine ehrlich ungenaue besser als eine, die auf einer Maschine kippt.**
- **Eine Regel im Kopf der Sitzung ist beim naechsten Start weg** · RECHNUNGSREGELN.md · **universell** — Adam fragte nach einer Reihe von Korrekturen an seiner Rechnung: *Ist denn das jetzt eigene Regeln oder gelten die dann genauso fuer Claudia?* Die ehrliche Antwort war: bis dahin galten sie **nur hier**. **Tatsaechlich eingetretene Nebenwirkung:** Beim Aufschreiben stellte sich heraus, dass es **zehn** Regeln waren, nicht drei — Sammelposition statt Einzelposten, kein Verweis aufs Beiblatt, Referenzzeile nur bei echter Nummer, Prozente statt Texte, Rabatte ausgewiesen, Tagessatz je Auftraggeber. **Keine davon haette den naechsten Sitzungsstart ueberlebt.** Adams zweite Auflage war die wichtigere: *ergaenzen, nicht ueberschreiben*. **Lehre: Wer Regeln aufschreibt, schreibt dazu, was sie NICHT ersetzen** — sonst wird aus einer Ergaenzung stillschweigend ein Widerruf.
- **Der Pruefer der Zielumgebung hat meinen eigenen Fehler gefangen, waehrend ich ihn erklaerte** · Z-1b · **universell** — Ich schrieb `${RECHNUNGEN_AUSGANG:-$HOME/...}` in den Tagescheck und zitierte im selben Block die Lehre vom 29.07. Der Laeufer meldete sofort: *als root-Dienst ist HOME leer, set -u bricht ab* — dieselbe Zeile, die einundzwanzig Tage lang lautlos einen Waechter getoetet hatte. Zwei Zeilen ueber meiner Aenderung stand `BOTHOME=/home/claudebot` **mit genau diesem Vorfall im Kommentar**. **Lehre: Eine Lehre zu kennen schuetzt nicht davor, sie zu wiederholen — ein Pruefer schon.** Deshalb sind sie gebaut und nicht aufgeschrieben.
- **Eine Quittung, die niemand liest, ist eine Logzeile** · Befund 4 · **universell** — Der Log-Abgleich meldet, was er mitgenommen hat und was nicht. Beim Ausschluss des Rechnungszweigs gab es genau zwei Arten, das falsch zu machen, und beide waren in diesem Projekt schon einmal da: **lautlos aussortieren** (dann faellt niemandem auf, dass ein ganzer Zweig fehlt) oder **Datei fuer Datei melden** (dann steht unter jeder Rechnung *bitte melden, das sollte mitkommen* — 2026-08-20 mit 120 Pip-Dateien passiert). **Tatsaechlich eingetretene Nebenwirkung:** Der Transport-Fix allein haette die zweite erzeugt; der Bericht musste im selben Zug gespiegelt werden. Geloest mit **einer** Sammelzeile samt Grund. **Lehre: Wer einen Filter aendert, aendert zwei Dinge — was transportiert wird und was darueber berichtet wird.** Das eine ohne das andere ist entweder Stille oder Laerm.
- **Ein Ausschluss ist ein Datenschutz-Bauteil, kein Aufraeumen** · Befund 4 · **universell** — Adams Rechnungsprojekt zog am 03.09. in den Arbeitsordner, der stuendlich in ein GitHub-Repo abgeglichen wird. Die PDFs tragen **Bankverbindung und Steuernummer**; `--include='*.pdf'` haette sie mitgenommen, und die Historie eines Repos vergisst nichts von selbst — das Entfernen kostet danach einen Force-Push samt Nachziehen aller Klone. **Lehre: Wer einen Ordner in einen abgeglichenen Baum legt, prueft im selben Zug den Filter.** Der Umzug war der Zeitpunkt, an dem das billig gewesen waere.
- **Ein Pruefstand, der nur den ERSTEN Lauf misst, findet nur Fehler des ersten Laufs** · Befund 1 · **universell** — Fuer den Ablegeweg gab es drei Gegenproben (Elternordner fehlt · Ziel nicht anlegbar · leerer Ordner). Alle drei pruefen denselben Moment: den ersten Durchgang. Der Fehler sass im **zweiten**: Gezaehlt wurde der Bestand im Uebergabeordner, der nie geleert wird, also meldete jede Mac-Sitzung erneut *N Rechnungen gelegt* — mit wachsendem N und ohne dass etwas geschah. **Tatsaechlich eingetretene Nebenwirkung:** Die Bedingung *nur melden, wenn etwas ankam* existierte und war richtig gedacht; sie mass nur die falsche Groesse. **Lehre: Zu jedem zustandsbehafteten Ablauf gehoert die Zeile ,zweimal laufen lassen, der zweite ist still'** — und ihre Gegenrichtung, sonst baut man Stille statt Genauigkeit.
- **Wer eine Befehlszeile aus fremdem Text baut, hat schon verloren — der Ausweg ist ein anderer Weg, nicht bessere Maskierung** · Befund 3 · **universell** — Der Fernbefehl setzte einen Ordnernamen in einfache Anfuehrungszeichen. Ein Apostroph (`L'Osteria`) beendete sie; heute fiel nur die Nachricht aus, mit `x'; <befehl>; echo '` waere der Rest als `claudebot` gelaufen. **Tatsaechlich eingetretene Nebenwirkung:** In der Gegenprobe entstand die Beute-Datei wirklich — der Angriff war keine Theorie. Geloest ueber **stdin** (`--text -`), wo die Gegenseite nichts zu deuten hat; die Chat-Kennung wird als Zahl **geprueft** statt maskiert, damit in der Zeile gar kein ungepruefter Text mehr steht. **Lehre: dieselbe wie beim Heredoc fuer Commit-Nachrichten — wer die Ausnahme begruenden muss, begruendet sie irgendwann falsch.**
- **Eine Pruefzeile kann die Darstellung messen statt der Sache** · Nachpruefung 04.09. · **universell** — Vorgeschlagen war `grep -c "017-26"` gegen den Rechnungszaehler auf dem Server; das Ergebnis war **null**, und daraus folgte scheinbar der schlimmste Fall (die Nummer wird ein zweites Mal vergeben). Gemessen war das Gegenteil: Der Zaehler speichert die **nackte 17**, nicht die Nummer als Text, und Mac und Server sind zeichengleich. **Lehre: Eine Pruefzeile gegen eine Datei muss deren FORMAT kennen** — sonst misst sie die Schreibweise und meldet einen Fehlalarm, der wie ein Alarm aussieht.
- **Eine mechanische Ersetzung ist keine Umstellung — der Vorgabewert verdeckt sie** · Block 1b · **universell** — Beim Schluesselwechsel wurde `SESSIONS.get(user_id)` durch `_sess(user_id)` ersetzt, sauber und an allen Stellen. Weil `thread_id` einen **Vorgabewert** hat, lief jede dieser Stellen weiter — auf dem Hauptfaden. **Tatsaechlich eingetretene Nebenwirkung:** Die Pruefzeilen blieben alle gruen, weil sie die Traeger messen (Schluessel, Schlangen, Sitzungen) und nicht die Wege darueber; gefunden hat es erst eine fremde Nachpruefung, sechs Stellen auf einmal. **Lehre: Ein Vorgabewert, der ein altes Verhalten weiterlaufen laesst, macht den Umbau bequem und die Luecke unsichtbar.** Der Ausweg ist kein besserer Blick, sondern eine Mengen-Pruefzeile: alle Aufrufe mit fehlendem Argument zaehlen und je einen begruendenden Kommentar verlangen. Dann ist jede verbleibende Stelle eine Entscheidung.
- **Wer eine Quelle teilt, teilt sie auch fuer alle, die sie lesen** · Block 2 · **universell** — Das Protokoll je Zimmer schien eine reine Erweiterung: eine Datei mehr, niemandem etwas weggenommen. **Tatsaechlich eingetretene Nebenwirkung:** Der Log-Wachposten liest genau `<datum>.md`. Arbeit in einem Zimmer waere fuer ihn unsichtbar geblieben, und er haette **Stille gemeldet, waehrend gearbeitet wird** — die eine Fehlerrichtung, gegen die er gebaut ist. Gefunden vor dem Bauen, weil die Geschwister-Regel verlangt, die Leser einer Quelle zu benennen statt sie zu denken. **Lehre: Eine Quelle aufzuteilen ist kein Hinzufuegen — es ist eine Aenderung fuer jeden, der sie liest.** Der Hauptfaden behielt deshalb seinen Namen; nur der Wachposten musste lernen, dass es jetzt mehrere sind.
- **Ein `try/except` um eine Entscheidung macht jeden Fehler darin zur Ablehnung** · Freigabeweg 10.09. · **universell** — Die Dialog-Messung kam in die Klammer, die das Senden absichert. Ihr `NameError` fiel in den `except`, und der antwortet mit einer Verweigerung. **Tatsaechlich eingetretene Nebenwirkung:** Der Dialog kam bei Adam an, er druckte „Genehmigen", und das Werkzeug war trotzdem verweigert — fuenfeinhalb Stunden lang, ohne dass ein Fehler sichtbar wurde. Das ist der **zweite** Fall dieser Klasse in drei Wochen; beide Male grüner Regressionslauf, beide Male ein `except`, das den Bruch in Ruhe verwandelt. **Lehre: Buchfuehrung sitzt nie in der Klammer, die eine Entscheidung traegt** — und gegen die Klasse hilft kein weiterer Verhaltenspruefer, sondern eine statische Namenspruefung, die in einer Sekunde alle Pfade sieht, die kein Test ausfuehrt.

- **Werkzeugloser Empfang mit genau einem Ausgang** · Block 3 (10.09.2026) · universell — Kettenwirkung geprüft an `werkzeugfreie_optionen` (zwei Nutzer, Positivliste als Parameter mit leerer Vorgabe). Tatsächliche Nebenwirkung: Die Trennung „Fabrik plus ein Eintrag" deckte auf, dass **der volle MCP-Name Sicherheitslogik ist, keine Beschriftung** — mit dem Kurznamen verweigert `dontAsk` das eigene Werkzeug, und die Rolle wäre stumm, ohne dass jemand einen Fehler sähe.
- **Ein typisierter Aufruf statt eines Textmarkers** · Block 3 · universell — die Bruchstelle „zitierter Fremdtext wird zum Auftrag" verschwindet bauartbedingt statt durch einen Filter. Nebenwirkung: Der Riegel gegen den **doppelten** Auftrag musste trotzdem Code werden (`nur_antworten`); ein Satz im Prompt wäre eine Bitte gewesen.
- **Registerschlüssel getrennt von der Telegram-Nummer** · Block 3 · anpassbar — beim Bauen aufgefallen: `int(None or 0)` hätte jeden Auftrag ohne Telegram-Nummer unter der Null geführt und den nächsten still übersprungen. Der Fehler lag seit Block 1b latent und wäre erst mit dem ersten Auftrag aus dem Empfang aufgeschlagen.
- **Ein Wächter, der Dateinamen misst, trifft Namensgleichklänge und verfehlt Umbenennungen** · Block 3 (10.09.2026) · universell — gemessen beim Bau: Die Zeile „Log-Repo-Ampel (5.19)" schlug auf `sekretariat.py` an, das mit Rechnungen nichts zu tun hat. Die richtige Antwort war **umbenennen** (`empfang.py`), nicht den Wächter erleichtern: Der Name war tatsächlich für 5.19 belegt. Die gefährlichere Richtung bleibt offen und ist gemeldet — dieselbe Liste findet `rechnungslauf.py` nicht.
- **Grenzen und Zeiten als Einstellgröße, Entscheidungen als eigene Funktion** · Block 3 (10.09.2026) · universell — Kettenwirkung geprüft an Worker-Schleife und Stall-Wächter. Tatsächliche Nebenwirkung: Erst durch das Herausziehen von `darf_starten`/`darf_einschlafen` wurde die Selbstzähl-Falle sichtbar — ein Zimmer, das sich selbst mitzählt, drosselt sich bei Grenze eins sofort aus. In der Schleife hätte das keine Prüfzeile erreicht.
- **Ein Schlüssel ist ein Wert, kein Satz von Teilen** · F-22 (11.09.2026, Klon `probe-f22`) · **universell** — Nach Block 1b trugen alle Türen `user_id` und `thread_id` einzeln; der Chat fehlte, und genau das war die Lücke. Gebaut wurde deshalb kein drittes Argument, sondern ein **benannter Typ**: `Faden(person, chat, thema)` als NamedTuple, und die Türen nehmen **einen Faden**. 27 Index-Zugriffe wurden auf Namen umgestellt, weil ein viertes Feld sonst jede `fd[1]`-Stelle still verschiebt. **Tatsächlich eingetretene Nebenwirkung, und sie ist der Ertrag:** Der Umbau legte **zwei vorbestehende Fehler** offen, die ohne die Trennung nicht auffallen konnten — der Reaktions-Widerruf griff über `sess.chat_id` in den falschen Faden (ein zurückgenommener Daumen lief still ins Leere), und `zimmer_ziel` **riet** beim Hauptchat, der Chat sei die Person. **Lehre: Wer eine Lücke schließt, indem er ein Argument ergänzt, schließt sie einmal; wer sie schließt, indem er die Teile zu einem Wert macht, schließt sie für jede künftige Tür.**
- **Eine Prüfzeile, die Argumente zählt, überlebt den Umbau nicht — eine, die die Bauform misst, schon** · F-22 · **universell** — Die Mengen-Zeile aus Block 1b zählte Aufrufe mit fehlendem `thread_id`. Mit dem Faden als einem Wert war sie gegenstandslos: Es gibt kein fehlendes Argument mehr, also blieb sie grün und maß nichts. **Tatsächlich eingetretene Nebenwirkung:** Der bequeme Weg wäre gewesen, sie zu löschen — die Zusage dahinter gilt aber weiter. Sie wurde deshalb **umgestellt, nicht entfernt**: Gemessen wird jetzt, dass **keine Tür Person und Thema einzeln nimmt**. **Lehre: Wenn eine Prüfzeile durch einen Umbau gegenstandslos wird, ist die Zusage zu suchen, die sie trug** — sie ist fast nie mit verschwunden.
- **Eine Prüfzeile, die den Gegenstand am Text erkennt, sieht nur die Hälfte** · Schalterstand im Menü (11.09.2026) · **universell** — Gebaut war die Gegenrichtung wie beauftragt: Jeder Befehl, dessen Beschreibung „an/aus" oder „umschalten" enthält, muss in `_SCHALTER` stehen — damit der nächste Schalter nicht ohne Stand entsteht. **Tatsächlich eingetretene Nebenwirkung, beim Bauen gemessen:** Von den sechs heutigen Schaltern verraten sich **drei nicht** — `technik` heißt „Klartext ↔ Rohform", `quiet` heißt „Tipp-Indikator aus", `verbose` heißt „wieder an". Die Zeile war grün und hätte die Hälfte ihrer Fälle nie gesehen. Ergänzt um eine **Mengen-Zeile**: Welcher Befehls-Handler legt einen Zustand UM (`x = not …`)? Das Befehl-Handler-Paar kommt aus den `CommandHandler`-Registrierungen, nicht aus einer gepflegten Liste — vier Treffer, alle schon eingetragen, kein Fehlalarm, also keine Ausnahmeliste nötig. **Lehre: Wer eine Menge über die BESCHREIBUNG bildet, misst, wie sorgfältig jemand benannt hat — nicht, was der Code tut.** Und die verbleibende Lücke gehört benannt statt überbaut: `/quiet` und `/verbose` sind ein Paar, kein Umschalter, und fielen durch beide Raster; ein dritter Prüfer dafür wäre der Wächter dritter Ordnung.
- **Ein Ausschluss, der am Punkt hängt, hängt nicht an der Sache** · A5 Log-Abgleich (11.09.2026) · **universell** — `--exclude='.*'` hielt `.venv` aus dem Log-Repo heraus. **Tatsächlich eingetretene Nebenwirkung, am Prüfstand gemessen:** `venv/lib/README.md` und `node_modules/paket/README.md` kamen **mit** — Markdown aus fremden Paketbäumen lag im Repo. Der Ausschluss traf eine Schreibweise, nicht eine Art von Ordner. Dritter Fall derselben Klasse in diesem Projekt: am 03.09. griff `.venv` im Backup nicht, weil der Ordner anders hieß; am 25.07. gewann ein Include über einen Ausschluss, der dahinter stand. **Lehre: Nach jedem Filter-Eingriff wird gemessen, was im ZIEL liegt** — und die Geschwister-Regel gilt auch für Filter: Wer einen Pfad ausschließt, benennt, welche anderen dasselbe Problem haben.
- **Wer einen Kommentar zwischen Fortsetzungszeilen setzt, ändert das Kommando** · A5 · **universell** — Der Geschwister-Ausschluss bekam eine Erklärung, und die rutschte zwischen die mit `\` fortgesetzten `rsync`-Argumente. Der Kommentartext **wurde zum Argument**; der Transport lieferte danach gar nichts mehr. **`bash -n` sagt dazu nichts — es ist gültige Syntax.** Gefunden hat es eine Prüfzeile, die eine Minute vorher entstanden war: *„Papier und Gesprächslog sind trotzdem angekommen"*. **Lehre: Zu jedem Sparsamkeits- oder Filtergriff gehört die Gegenzeile, dass das Richtige noch ankommt** — ein Filter kann nur zu viel oder zu wenig durchlassen, und geprüft wird meist nur eine der beiden Richtungen.
- **„Deine" war richtig, bis eine zweite Stimme dazukam** · A1 Kontext-Zeile (11.09.2026) · **universell** — Der Reply-Kontext sagte jedem Leser *„deine vorherige Nachricht"*. Das stimmte, solange genau eine Instanz im Chat sprach. **Tatsächlich eingetretene Nebenwirkung, in Adams Chat gemessen:** Seit es den Empfang gibt, bekam die Sekretärin Nachrichten des Hauptfadens als ihre eigenen vorgestellt — sie kannte sie nicht, fragte zurück und bremste zweimal, völlig korrekt nach ihrem eigenen Prompt. Der Satz war nicht falsch gebaut, er ist **falsch geworden**. **Lehre: Wer eine zweite Instanz in einen bestehenden Kanal setzt, muss jede Formulierung prüfen, die „du" oder „deine" sagt** — sie trägt eine Annahme über die Anzahl der Sprecher, die niemand aufgeschrieben hat. Die Lösung war nicht, den Leser zu kennen, sondern den **Urheber zu nennen**: Ein Satz, der den Urheber benennt, ist für jeden Leser wahr.
- **Eine Prüfzeile, die ihren Fall abwartet statt ihn herzustellen, misst den Zufall** · A4 (11.09.2026) · **universell** — Die Zeile „ein leerer Ordner gilt nicht als Schriftordner" prüfte, ob der **gefundene** Ordner Schriften enthält. Sie war grün, und ihre Gegenprobe (Inhaltsprüfung entfernt) blieb es auch: Auf diesem Mac steht zufällig ein voller Ordner vorn in der Kandidatenliste. **Tatsächlich eingetretene Nebenwirkung:** Die Zeile hätte auf einer anderen Maschine gemessen und hier nie — schlimmer als keine Zeile, weil sie Sicherheit vortäuscht. Umgestellt: Der leere Ordner wird über `XDG_DATA_HOME` **an den Anfang der Liste gesetzt**, dann misst sie. **Lehre: Wenn eine Gegenprobe nicht rot wird, ist die Prüfzeile der Befund — nicht die Gegenprobe.**
- **Eine Freigabe, die von selbst endet, ist etwas anderes als eine, die man zurücknehmen muss** · Schreiben frei (11.09.2026) · **universell** — `Write` und `Edit` standen seit August auf der Nie-dauerhaft-Liste, und das Aufnahmekriterium war *„ein Klick gilt danach unsichtbar fort"*. Der neue Knopf hebt die Liste nicht auf, er erfüllt das Kriterium schlicht nicht: Das Flag lebt **nur im Speicher**, endet mit dem Prozess (täglicher Hygiene-Neustart), gilt **nur** unter dem Arbeitsordner, und der Knopf zeigt seinen Stand, solange er gilt. **Tatsächlich eingetretene Nebenwirkung beim Prüfen:** Die Gegenprobe „Flag in die Vorlieben schreiben" blieb zunächst grün — mein Eingriff hatte nur das Dict im Speicher geändert und war damit gar kein Eingriff. Erst mit echtem Speichern wurde die Zeile rot. **Lehre: Ein Eingriff, der dasselbe tut wie der Originalzustand, ist keine Gegenprobe** — und die Verwechslung sieht wie ein Ergebnis aus.
- **Ein Riegel kann einen anderen verdecken, und dann misst die Prüfzeile den falschen** · Schreiben frei · **universell** — Die Zeile „trotz Flag bleibt Gesperrtes gesperrt" enthielt einen `..`-Pfad, der aus dem Arbeitsordner in den Gedächtnis-Ordner führte. Sie war grün. **Gemessen in der Gegenprobe:** Entfernt man die Pfadauflösung (`resolve()`), bleibt sie **trotzdem** grün — der Fall wurde nämlich von der Geheimnis-Prüfung gefangen, nicht von der Pfadprüfung, die er messen sollte. Ein zweites, **harmlos benanntes** `..`-Ziel machte den Unterschied sichtbar. **Lehre: Wo mehrere Riegel hintereinanderliegen, braucht jede Prüfzeile einen Fall, den NUR ihr Riegel fängt** — sonst misst man die Summe und hält sie für den Einzelnen.
- **Eine Hypothese, die man messen kann, wird gemessen — und ihr Scheitern ist ein Befund, keine Leerstelle** · Unerklärte Löschung (23.09.2026) · **universell** — Aus dem Arbeitsbaum fehlten 21 Dateien, nicht committet. Engywucks Hypothese war stark: iCloud lagert aus, statt zu löschen. **Vier Messungen, keine traf** — das Repo liegt außerhalb des iCloud-Bereichs, die Speicher-Optimierung ist aus, keine Platzhalter, kein Git-Verweis bewegt. **Tatsächlich eingetretene Nebenwirkung:** Gerade weil die Hypothese scheiterte, las ich das iCloud-Protokoll bis zu seinem Anfang — und der lag am **15.09., 02:26**, eine Minute vor der einzigen fremden Änderung im Repo, in der Nacht eines **macOS-Großupdates**, von dem niemand wusste. Ohne die Gegenhypothese wäre nach dem ersten Fehlschlag Schluss gewesen. **Lehre: Eine widerlegte Vermutung zeigt oft auf die richtige Stelle, weil sie den Messgang dorthin zwingt.** Und eine zweite, die dazugehört: Das Zusammentreffen ist ein Anhaltspunkt, kein Beweis — die Pfade im Protokoll sind geschwärzt, und wer daraus eine Ursache macht, erfindet sie.
- **Eine Reichweite, die zuverlässig endet, kann genau zur falschen Zeit enden** · Block 1, ein Knopf (23.09.2026) · **universell** — Die Schreibfreigabe vom 11.09. lebte nur im Speicher, damit sie mit dem Hygiene-Neustart um vier endet. Das war als Sicherheitsgewinn gebaut und begründet. **Tatsächlich eingetretene Nebenwirkung, gemessen am 21.09.:** Um 04:30 begann die Arbeit, die Freigabe war weg, zwei Schreib-Dialoge warteten je eine Stunde ins Leere. Die Reichweite endete zuverlässig — und zwar genau dann, wenn gearbeitet wird. **Lehre: Wer eine Freigabe an einen Zeitpunkt bindet, muss prüfen, was um diesen Zeitpunkt herum geschieht** — nicht nur, dass sie endet.
- **Wer eine Funktion prüft, weiß noch nicht, ob sie gerufen wird** · Block 1 · **universell** — Der Prüfer maß `schreiben_ohne_frage` in vier Zeilen, alle grün. Die Gegenprobe „Arbeitsordner-Riegel entfernen" ließ dort den Repo-Klon durch — **weil der Repo-Schutz gar nicht in dieser Funktion sitzt**, sondern als harter Deny weiter oben im Rückruf. Die Übergabe nahm an, er liege in `_is_sensitive_ref`; das stimmt für `CLAUDE.md`, nicht für `bot.py`. **Tatsächlich eingetretene Nebenwirkung:** Eine siebte Zeile fährt jetzt den echten Rückruf. Die Gegenprobe „Schreib-Zweig aus dem Rückruf nehmen" macht **nur sie** rot — die vier Funktionszeilen blieben grün und hätten einen entfernten Aufrufer nie bemerkt. **Lehre: dieselbe wie am 22.08., an neuer Stelle — Fabrik ja, Aufrufer nein.** Sie kehrt wieder, weil die Funktion der bequemere Prüfgegenstand ist.
- **Eine Prüfung in einer Umgebung, die den Fehler nicht hat, kann nicht rot werden** · PDF-Skript, Schriften (23.09.2026) · **universell** — Am 11.09. wurde *„font fallback list must not be empty"* am Mac geprüft und für behoben erklärt; die Abnahme in der Zielumgebung war Claudia nach dem Deploy übertragen. Sie stieß am 21.09. darauf und nahm einen Umweg, am 23.09. stand der Fehler auf dem Server unverändert da. **Tatsächlich eingetretene Nebenwirkung, gemessen:** pandoc 3.10 am Mac baut die typst-Vorlage so, dass eine fehlende Hauptschrift nicht auffällt; pandoc 3.1.11 auf dem Server bricht ab. **Der Mac konnte den Fehler nicht zeigen — also konnte keine Prüfung dort rot werden, auch keine gute.** Und die Diagnose selbst war falsch: nicht der Ordner fehlte, sondern der Name. **Lehre: Vor der Frage „ist es behoben?" steht die Frage „kann meine Umgebung den Fehler überhaupt zeigen?"** — erst die Gegenrichtung (derselbe Lauf ohne den Fix muss scheitern) beweist, dass die Prüfung die Ursache trifft. Das ist R1, und es hat an einer Stelle nicht gegriffen, an der ich es selbst verschoben hatte.
- **Der Riegel sitzt nicht immer dort, wo die Bedingung steht** · Block 1b, Sammelnachricht (24.09.2026) · **universell** — Die Regel „ein Daumen entscheidet nur bei genau einer offenen Anfrage" steht als `if request_id == _SAMMEL:` im Reaktions-Handler. Die Gegenprobe entfernte diese Zeile und erwartete Rot bei „der Daumen entscheidet NICHTS" — die Zeile blieb grün. **Tatsächlich eingetretene Nebenwirkung:** Der eigentliche Riegel ist der Merkerwert selbst; `*` ist keine Kennung, also löst ein Daumen darauf nie etwas aus, mit oder ohne `if`. Die Zeile trägt nur den Hinweis an Adam. **Lehre: Eine Gegenprobe muss den Riegel treffen, nicht die Stelle, an der er erklärt wird** — sonst misst sie Beiwerk und hält es für die Sicherung.
- **Eine Umwandlung kann einen Fehler verstecken, den das Rohformat gezeigt hätte** · Block 2, Darstellung (24.09.2026) · **universell** — Die Gegenprobe „Steuerzeile nicht entfernen" ließ die Zeile *„erscheint nicht im Chat"* grün: Die Markdown-Umwandlung schluckte die spitzen Klammern, und der Prüfer suchte nur das Wort `vorschau`. **Tatsächlich eingetretene Nebenwirkung:** Die Adresse stand nackt im Chat, der Prüfer sah es nicht; erst die Nachbarzeilen wurden rot. **Lehre: Wer hinter einer Umwandlung prüft, prüft auf das, was übrig bleibt — nicht auf das Zeichen, das die Umwandlung ohnehin frisst.**
- **Eine Summe, die per Bauart stimmt, ist kein Prüfer** · Block 3, Adressat je Befund (24.09.2026) · **universell** — Claudias Vorschlag: je Lauf die Befunde je Klasse zählen und Alarm schlagen, wenn die Summe von der Zahl der erzeugten abweicht. Beim Bau gezählt, an welcher Stelle: in denselben Türen, die die Befunde verteilen — die Summe stimmt dann immer. **Tatsächlich eingetretene Nebenwirkung:** keine, weil der Alarm nicht gebaut wurde; gebaut wurde die Zählzeile fürs Protokoll und ein Pflichtfeld-Prüfer, der einen Befund ohne Tür am Quelltext findet. **Lehre: Ein Abgleich prüft nur, was auf zwei unabhängigen Wegen gezählt wird** — zählt beide Seiten dieselbe Stelle, misst er sich selbst.
- **Ein Rückfall braucht ein Gedächtnis, sonst wird er zur Schaukel** · Block 4, Modellwächter (24.09.2026) · **universell** — Eine Automatik, die von selbst umstellt und bei Scheitern von selbst zurückgeht, stellt am nächsten Morgen wieder um, sofern sie nicht weiß, dass sie es schon versucht hat. **Tatsächlich eingetretene Nebenwirkung:** Beim Bau vorhergesehen, nicht erlebt: Ohne die Liste `zurueckgenommen` hätte der Wächter jeden Morgen dieselbe Kennung eingetragen, der Bot sie bei der ersten Nachricht wieder verworfen und Adam täglich zwei Meldungen bekommen. Die Gegenprobe „Liste entfernen" macht genau diese Zeile rot. **Lehre: Jede selbsttätige Umkehr braucht einen Vermerk, dass sie stattfand** — sonst ist der Rückfall nicht das Ende des Versuchs, sondern sein Takt.
- **Wer einen Namen zum Schlüssel macht, muss beim Umbenennen jede Stelle kennen, an der er Schlüssel ist** · Block 5, Zimmerliste (24.09.2026) · **universell** — Claudias Auftrag zählte vier Stellen, an denen ein Zimmername Schlüssel ist. Gemessen waren es drei: Liste, Vorlieben, Routen. Die vierte (laufende Aufträge) hängt an der Thema-Kennung, und die Ordnerspiegelung gibt es im Code nicht. **Tatsächlich eingetretene Nebenwirkung:** Die Routen standen im Code und hätten beim Umbenennen gar nicht nachgezogen werden können — der Umbau musste sie mit in die Datendatei nehmen, obwohl der Auftrag nur die Häuser nannte. **Lehre: Bevor Daten aus dem Code wandern, alle Stellen zählen, die auf sie zeigen** — sonst zieht die schnelle Hälfte um und die langsame bleibt als stille Bruchstelle zurück.
- **Ein Dienst, den ein anderer startet, erbt dessen Benutzer — und damit die Besitzrechte an allem, was er anlegt** · Block 6, Zufluss (24.09.2026) · **universell** — Der Zufluss sollte „am bestehenden Monitor-Zeitgeber" hängen. Gemessen: Der Monitor läuft als root mit dem System-Python (Unit ohne `User=`). **Tatsächlich eingetretene Nebenwirkung:** Beim Bau vorhergesehen, nicht erlebt — hätte der Monitor den Zufluss direkt gerufen, gehörte der Eingang root, und der Bot hätte seinen Sichtungs-Merker nie setzen können; `/neues` hätte jede Woche denselben Stapel vorgelegt. Dazu die zweite Bindung: nur Standardbibliothek, weil das System-Python keine venv kennt. **Lehre: Wer sich an einen vorhandenen Lauf hängt, erbt dessen Benutzer und dessen Werkzeugkasten — beides wird gemessen, nicht angenommen.**
- **Eine Grenze mit Ablaufdatum steht auf der Startseite oft noch, wenn sie längst abgelaufen ist** · Block 6, Transkript-Dienst (24.09.2026) · **universell** — Alle Papiere rechneten mit 50 Abrufen je Stunde; so steht es auf der Startseite des Dienstes und in seiner Schnittstellen-Beschreibung. Erst die AGB sagen: befristet bis 19.09.2026, dauerhaft 20. **Tatsächlich eingetretene Nebenwirkung:** Hätte der Bau mit 50 gerechnet, wäre die Stundengrenze beim ersten größeren [mehr auswerten] nach einem Drittel erreicht gewesen, und die Meldung hätte Adam eine falsche Zahl genannt. **Lehre: Eine Zahl, auf die gebaut wird, kommt aus dem verbindlichen Text (AGB), nicht aus dem werbenden** — und die Lesepflicht vor dem Bau ist genau dafür da.
- **Stille als Regel, Zählung als Wächter** · Block 3 Teil 2 (24.09.2026) · universell — Ein sauberer Neustart schweigt; der Bot vermerkt ihn, der Tagescheck zählt nur (Fenster statt Löschen, weil er als root läuft). Kettenwirkung geprüft: die [STILL]-Marke der Hygiene, die [AUTORUN]-Teile, Adams /restart. Tatsächliche Nebenwirkung: Die Stille gilt auch für jeden Deploy per `systemctl restart` — die Startnachricht war bisher Adams Erfolgsbeleg nach einem Deploy, der fällt bei sauberem Start weg.
- **Ein Trockenlauf hat so viele Ausgänge, wie es Stellen gibt, die Zustand ändern — nicht nur die, die nach außen senden** · F-Punkt Belegkette (24.09.2026) · universell — Der Tagescheck-Trockenlauf sperrte Postfach, Telegram und Protokoll, aber nicht das Rollen der Belegkette. Tatsächliche Nebenwirkung: Der Prüflauf rollte die echte Kette, und der echte Lauf meldete Sekunden später einen Fehler, den der Prüflauf selbst erzeugt hatte. Der F-Punkt „nie gerollt" war dadurch falsch benannt: Es wurde gerollt, nur an einer Stelle, deren Protokoll niemand schreibt.
- **Wiederaufnahme nach Kontingent-Stopp: der Wecker steht vor dem Stopp, nicht danach** · Zettel F1 Teil 2 (24.09.2026) · plattformgebunden (App-Wecker, Nutzungsanzeige) — Nach dem Stopp kann die Sitzung nichts mehr tun, also wird die Rückstellzeit vorher gelesen (`get_usage`) und ein einmaliger Wecker darauf gestellt; der Auftrag des Weckers endet sofort, wenn der Laufplan nichts Offenes führt. Tatsächliche Nebenwirkung: Beim Messen stellte sich heraus, dass der Mehrverbrauch abgeschaltet ist, am Limit also nichts abgebucht wird — die 💰-Frage ist damit an dieser Stelle beantwortet, solange der Schalter aus bleibt.
- **Eigenes Material braucht einen eigenen Ordner, nicht nur einen eigenen Weg** · Kurs-Videos (24.09.2026) · universell — Der Weg Mac → VPS war lokal und ohne Fremddienst gebaut; die Nebenwirkung lag daneben: Die Wissensablage reist mit dem Log-Abgleich nach GitHub, und ihr gemeinsamer Index hätte jeden Kurstitel mitgenommen. Erst ein eigener Unterordner mit eigenem Index ließ sich als Ganzes ausschließen — und der Quittungsbericht musste den Ausschluss spiegeln, sonst hätte er jedes Transkript als „bitte melden" geführt.
- **Ein Gestaltungsblatt trägt nur, was die Vorlage nicht selbst setzt** · PDF-Gestaltungsblatt (26.09.2026) · universell — pandoc 3.1 fügt Kopfzeilen hinter die Grundeinstellungen seiner Vorlage ein, 3.10 davor; ein Blatt, das dieselben Eigenschaften setzt, gewinnt je nach Fassung oder verliert. Tatsächliche Nebenwirkung: Die Wahl einer Serifenlosen, die nur der Server hat, hebt das gleiche Bild auf beiden Maschinen auf — bewusst, weil der Server der Ort ist, an dem Papiere entstehen.
- **Ein Kostenriegel bucht vor dem Aufruf, nicht danach** · 9.1 Azure (26.09.2026) · universell — Ein Aufruf, der scheitert, ist womöglich trotzdem berechnet; wer nach dem Erfolg bucht, zählt zu wenig, und der Riegel greift zu spät. Tatsächliche Nebenwirkung: Die Zahlen-Umschreiber mussten aus der Vorlese-Säuberung heraus in eine eigene Funktion, weil der Rückfall auf die alte Stimme sie erst an der Sendestelle braucht — die Weiche sitzt damit an zwei Stellen, die zusammen gehören.
- **Ein eingeschränkter Schlüssel ist nur so eng wie der Aufruf, der ihn benutzt** · Block 7, Mac-Weg (26.09.2026) · universell — rrsync bindet einen Schlüssel auf dem Server an einen Ordner. Tatsächliche Nebenwirkungen, alle erst beim Messen sichtbar: Ein nur lesender Eingang kann nicht aufräumen (also räumt der Server), das Mac-rsync spricht die Sprache von rrsync nicht (also rsync 3 — und damit eine neue Fassung für drei andere Skripte), und der gewohnte SSH-Eintrag hätte den vollen Schlüssel mit angeboten, sodass der Weg ohne den engen Schlüssel still mit vollem Zugang gelaufen wäre. **Lehre: Eine Beschränkung auf der Gegenseite schützt nur, wenn die eigene Seite nichts anderes anbieten kann.**
- **Ein Ausgang heißt: jede Sendestelle ist gleich — auch die, die Befehle zeigt** · Block 2b (26.09.2026) · anpassbar (plattformgebunden an python-telegram-bot, das Muster universell) — Die Unterklasse des Bot-Objekts machte zweihundert Stellen auf einen Schlag schöner. Tatsächliche Nebenwirkungen: Der Freigabedialog wäre beim Nachziehen ausgezeichnet worden (ein Befehl `a*b*c` hätte anders ausgesehen, als er lautet), und die Umwandlung verschluckte spitze Klammern still — beides nur gefunden, weil vor dem Breitmachen am Paket gemessen und jede Stelle gezählt wurde, die Wörtlichkeit braucht. **Lehre: Wer einen Weg für alle baut, braucht einen ausdrücklichen Weg für „genau so, wie es dasteht" — und einen Wächter, der Verlust als Klasse erkennt.**
- **Die Stimme ist das Signal — und die Einstufung gehört zur ganzen Antwort** · 9.2 lokale Stimme (26.09.2026) · universell — Eine zweite, hörbar andere Stimme macht einen Datenweg für den Menschen wahrnehmbar, ohne dass er liest. Tatsächliche Nebenwirkungen: Die Einstufung je Teilstück hätte den Sprecher mitten in der Antwort wechseln lassen und ein harmloses Teilstück einer roten Antwort an die Cloud gegeben — also wird die ganze Antwort eingestuft. Und die Zahlen-Umschreiber der alten Stimme verschlechterten die neue: Jede Stimme braucht ihre eigene, am Ohr gemessene Aufbereitung. **Lehre: Wer ein Sicherheitsmerkmal hörbar macht, muss es auch durchgehend halten — ein Signal, das mitten im Satz kippt, lehrt das Falsche.**
- Eine Erkennung je Frage fuer alle Ausgabewege (Jahreszahl: edge, Piper, Azure rufen `jahreszahl.art`) · 26.09. (88) · universell — Nebenwirkung: der gemeinsame Bau deckte in einem Weg den umgekehrten Fehler auf (Azure las jede Zahl als Jahr).
- Eine Stimme je Text, Grenze je Sprecher statt je Transport · 26.09. (86) · anpassbar (Telegram-Grenze 1024 fuer Bildunterschriften) — Nebenwirkung: zwei Pruefer setzten das Teilen stillschweigend voraus.
- **Dirigent Baustein 1: der Nebenfaden — Schluessel und Ausgabe-Adresse getrennt** · 5.1 / (90) (26.09.2026) · universell — Eine zweite, kurzlebige Sitzung fuer dieselbe Adresse beantwortet, waehrend die erste rechnet; der eingereihte Zwilling bleibt die Sicherung bis zur belegten Zustellung. Tatsaechliche Nebenwirkungen: Der Schluessel einer Sitzung war zugleich die Telegram-Themenkennung (22 Ausgabestellen, 16 Schluesselstellen) — die Trennung ist Vorlauf fuer Stufe 2; ein Pruefer, der nur die Art einer Ablehnung prueft, blieb ohne Sperre gruen, weil ohne Sitzung ohnehin abgelehnt wird. **Lehre: Wer eine Sperre prueft, prueft ihren Grund, nicht nur ihr Ergebnis.**
