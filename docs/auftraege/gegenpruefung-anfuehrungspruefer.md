...
| davon echt unausgewogen | **4** — `bot.py:4777, 4808, 4870, 4878` · ~~+ `test_email_9_5.py:477`~~ **[BERICHTIGT: war mein Auswertungsfehler, siehe unten]** |
...
## `[BERICHTIGT nach 0b4d7bc]` Meine fünfte Stelle gab es nicht — und der Fehler ist derselbe

Mick meldet nach dem Fix **vier** echte Stellen, ich hatte **fünf** gezählt.
Nachgemessen: `scripts/test_email_9_5.py:477` ist eine **leere Zeile**.

Die Ursache liegt in meiner Auswertung, nicht in seinem Prüfer. Ich habe jeden
Treffer danach einsortiert, ob **die Zeile** ausgewogen ist — aber bei einer
mehrzeiligen Zeichenkette benennt die Startzeile des Tokens nicht den Inhalt.
Eine leere Zeile hat null Öffner, fiel damit aus meiner Fehlalarm-Bedingung
(`auf == zu und auf > 0`) heraus und landete stillschweigend unter „echt".

**Das ist zum vierten Mal in dieser Nacht dieselbe Form** — und diesmal bei mir,
in der Gegenprüfung selbst: *Eine Zahl aus einem frisch gebauten Auswerter ist
noch kein Befund.* Ich habe Micks 70 an einer eigenen Nachbildung geprüft,
statt an dem, was der Prüfer tatsächlich misst. Genau der Griff, den ich ihm
zwei Absätze weiter oben als Lehre aufgeschrieben habe.

**Was dadurch NICHT falsch wird:** Die 64 Fehlalarme und die 32 betroffenen
Zeilen sind unabhängig davon belegt — an drei nachgesehenen, korrekt gepaarten
Zeilen und am Doppelmeldungs-Muster. Der Kernbefund („die 70 dürfen nicht
repariert werden") war richtig und hat den Schaden verhindert.

**Der Stand nach `a2ba359`, selbst gefahren, beide Versionen:**

```
py3.11.15 -> 4 Stellen: bot.py:4777, 4808, 4870, 4878
py3.13    -> 4 Stellen: bot.py:4777, 4808, 4870, 4878
```

**Identisch, keine Doppelmeldung.** Damit ist die eigentliche Zusicherung
erfüllt — nicht „vier", sondern **beide Umgebungen sehen dieselbe Menge**. Die
Zahl darf sich beim nächsten Commit ändern; die Gleichheit nicht.
