...
- **Status:** LÄUFT — Sicherung läuft verifiziert (16.07., vorgezogen laut Audit 2→3), **aber: RESTORE-PROBE offen** → erst damit gilt 4.1 als **voll** verifiziert (Adam-Auftrag 17.07.).
- **⚠️ Offen — Restore-Probe (Adam 17.07.):** Ein Backup ist erst bewiesen, wenn die **Rückrichtung** funktioniert. Durchzuführen: **Ampel-Regeldatei + Gedächtnis aus der Mac-Kopie in ein Testverzeichnis wiederherstellen** (nicht über die Originale!), Vollständigkeit + Lesbarkeit prüfen (MEMORY.md-Index auflösbar? `ampel_rules.toml` per `tomllib` parsebar?), **Ergebnis hier dokumentieren**. Danach Status → VERIFIZIERT. Prüfkriterium wandert ins Abhängigkeits-Register.
...
- **Status:** LÄUFT — **v1 LIVE seit 17.07.2026 (`91950ef`), Adam-Grundtests bestanden.** VERIFIZIERT erst mit v2 (Bezugs- + Sicherheits-Check).
...
- **⚠️ Offen (klein, aus den Tests):** (a) **Werkzeug-Spur unsichtbar**, weil `quiet: bool = True` der Standard ist (`if not sess.quiet`) — durch das Puffern sieht Adam bei langen Turns jetzt **gar kein Lebenszeichen** mehr (vorher wenigstens Textblöcke). Adam: „nicht so wichtig", aber Regress → Fix-Optionen: 🔧-Spur unabhängig von `quiet` zeigen oder Telegram-„tippt…"-Indikator. (b) `/presend` ist registriert (Zeile 4579) — Adam hatte `/present` getippt; ggf. `setMyCommands` ergänzen, damit Telegram den Befehl vorschlägt.
