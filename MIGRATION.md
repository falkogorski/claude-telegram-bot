...
- **➡️ AKTUELLER ARBEITSSTRANG (Stand 23.07.2026, nach dem Morgen-Deploy):** **5.9 + 5.25 + UI-Feinschliffe sind DEPLOYT** (Selbstcheck 16/16 auf dem VPS, Emoji-Gegenprobe bestanden). **Jetzt dran: Adam-Tests** — 🔴 **Härtetest drei schnelle Sprachnachrichten (Erinnerung ist raus)**, dann 5.9-Reaktionstest (👍 auf eine Bot-Frage) und 5.25-Tests (1)–(6). Danach **8.6 (b)** (Prüfskript `check_hilfe_buttons.py`), dann **8.7** (VPS-Schreibschutz technisch), erst danach Phase 3 (LobeChat, mit Pflicht-Zwischen-Audit). Trainer-PDF: Kontrollsitzungs-Prüfung BESTANDEN, als zweites Referenz-Artefakt in `docs/referenzen/`. Mitlaufend: Blaupause-Sammelpflicht je Punkt.
...
- **Status:** 🔄 **DEPLOYT (23.07. früh, Selbstcheck 16/16 auf dem VPS)** — es fehlt nur noch der Adam-Test (Drehbuch-Test unten)
