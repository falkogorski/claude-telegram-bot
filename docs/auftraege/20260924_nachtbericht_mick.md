...
- **Kurs-Videos Mac → VPS: gebaut, im Zweig `probe-kurse` (`8eb57fc`, 94/94), nicht im Hauptstrang.** Der Hauptstrang bleibt für deinen Deploy heute Abend unverändert: **Nach `748239a` kommen dort nur noch Papiere (`docs/`, Blaupause-Notizen, `CLAUDE.md`) und am 26.09. eine berichtigte Prüfzeile, kein Bot-Code.** Die Prüfzeile muss mit: Sie hing am Datum ihres Bautags und war ab dem 26.09. rot; ohne sie meldete der Tagescheck nach dem Deploy einen roten Regressionslauf. Den Kurs-Zweig führe ich erst nach deinem Deploy zusammen (Engywuck: *nach dem Deploy*). Danach genügt am Mac ein Aufruf: `bash scripts/mac/kurse_hochladen.sh <Ordner>`. Das überträgt als claudebot, transkribiert lokal und meldet sich über den Bot. **Adams Material geht nicht nach GitHub:** Die Transkripte liegen unter `wissen/kurse/`, und genau dieser Ordner ist aus dem Log-Abgleich genommen. Echte Probe am Mac: 4 Sekunden, Mitschrift korrekt.
...
1. **`probe-gesamt` liegt jetzt in `mac-produktivstand`** (`458b84a`, Merge-Commit, 92/92, gepusht). **Deployen kannst du ab jetzt einfach den Hauptstrang.** Schritt 0: `git log --oneline d54b272..origin/mac-produktivstand` zeigt **45 Commits** (42 beim Merge, dazu drei danach). **Zuerst die Abo-Probe für Opus 5.5** (Punkt 1 unten), weil Block 4 darin steckt, dann `pip install -r requirements.txt`. Der SDK-Pin ist **nicht** dabei.
2. **Block 3, Teil 2** (`f1e233e`, 93/93): Ein sauberer Neustart schweigt, gleich aus welchem Grund. Laut bleiben dein eigener `/restart` und alles mit Befund. Der Tagescheck zählt die stillen Neustarts; ab dem dritten am Tag bekommst du morgens eine Meldung mit den Gründen. **Das gilt auch für deinen Deploy:** Läuft er sauber, kommt keine Startnachricht mehr. Der Beleg steht im Journal: *Neustart sauber — Startmeldung unterdrückt*.
3. **F-Punkt „Belegkette nie gerollt"** (`748239a`): Gemessen, sie ist doch gerollt, am 02.09. und am 16.09. Gerollt hat aber der Prüflauf im Tagescheck, und der echte Lauf hat dir danach an beiden Tagen *❌ Es gibt noch keine Kette* geschickt. Das war ein Fehlalarm, den der Prüflauf selbst erzeugt hat. Jetzt rollt nur noch der echte Lauf; der nächste Rolltag ist der 30.09.
4. **Wiederaufnahme nach Kontingent-Stopp:** Den Kontingentstand kann ich jetzt ablesen, samt Rückstellzeit. **Mehrverbrauch ist abgeschaltet**: Am Limit bleibt die Sitzung stehen, abgebucht wird nichts. Der Wecker muss vor dem Stopp stehen; der Ablauf steht in `20260924_wiederaufnahme_mechanik.md`. **Heute läuft kein Wecker**, weil nichts Offenes mehr ohne dich geht.

**Liegt:** Block 5, Auftrag 4b (nach dem Deploy von Block 2) und der F-Punkt „Gedächtnis-Pfad im Prompt". Zu dem steht nur das Stichwort in der Übergabe vom 23.09., in der Nachtlese vom 19.09. steht nichts dazu.
