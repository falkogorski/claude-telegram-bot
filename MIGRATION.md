...
- **Status:** 🔄 **GEBAUT + LIVE (23.07.):** drei Schutzschichten verankert — Callback-Deny (Edit/Write), `_is_repo_write_cmd` für Bash (inkl. `-C`/Redirect/sed-Lücken, vom eigenen Selbstcheck gefunden), `pre-commit`-Blocker im VPS-Klon (live getestet, Exit 1); Selbstcheck-Zeile „Repo NUR-LESEN" prüft bei jedem Start. Offen nur Adams Abnahme beim Audit.
