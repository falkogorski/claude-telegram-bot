...
# Gegenprüfung der Bau-Sitzung — 31.08.2026, 02:0x–03:0x

**Angehängt, nicht eingearbeitet.** Der Text oben bleibt Engywucks Fassung; was
meine Messung anders ergibt, steht hier daneben. *Ein Befund, der stillschweigend
in fremde Arbeit hineinkorrigiert wird, ist hinterher nicht mehr von ihr zu
unterscheiden.*

## Bestätigt

| Befund | Womit gemessen |
|---|---|
| **Die verlorene Regel fehlt** | Drei verschiedene Formulierungen gegen `CLAUDE.md` **und** `MIGRATION.md` — *systemweit*, *Gesprächsverlauf*, *einzelne Sitzung*. Null Treffer. Eingetragen in `301b7c0`. |
| **7.4 war doppelt vergeben** | Beide Überschriften gelesen (Z. 941 / Z. 1072). Aufgelöst in `9941904`. |
| **7.1 ist kein Zimmer** | Die 6.6-Struktur führt vier Häuser, dreizehn Zimmer, plus den Bestand — der Erinnerungskanal kommt darin an keiner Stelle vor. |
| **Fünf Bausteine ohne jede Erwähnung** | Mit einem **anderen Raster** nachgesucht (alle 21 Wurzelmodule und alle Betriebsskripte, Dateiname **und** Klarname). Ergebnis: **nichts Zusätzliches.** Seine fünf tragen. |
| **Die sechs Falsch-Positive** | Alle sechs einzeln am Blocktext nachgemessen. Keiner ist ein echter Widerspruch — bei 5.19 stammt das Wort *fertig* aus einem anderen Projekt (`~/Projects/rechnungen`). |

## Korrigiert

**① 5.34 — der Befund trägt, seine Umsetzung hätte geschadet.** Gemessen ist der
Widerspruch echt: acht Stellen in `bot.py`, Register-Eintrag, Selbstcheck-Zeile
`✓ Große Dateien (5.34)` grün. **Aber der Punkt ist nicht fertig** — sein eigener
Blocktext sagt drei Zeilen weiter: *„Noch zu tun (root, Adam): Server installieren
und einschalten."* Wer ihn nach dem Befundsatz auf VERIFIZIERT gesetzt hätte,
hätte einen nicht installierten Dienst als fertig ausgewiesen. Richtig ist die
dritte Stellung: **gebaut, wartet auf Adam.** → `c807190`

**② 4.3 ist nicht die Ordnerspiegelung — der Befund ist gekippt.** Punkt 4.3
heißt *Memory + Configs in privates git-Repo (nicht iCloud)* und liegt in Phase 4.
Er hat mit den Telegram-Gruppen nichts zu tun und wird von ihnen nicht entblockt.
`git log -S` zeigt: Er hieß nie anders. **Die Ordnerspiegelung hat überhaupt
keinen Punkt** — sie lief bisher unter fremder Nummer, an zwei Stellen
(6.6-Statuszeile und Register Z. 107). → `29fe8d7`

**③ Die Zehn sind elf, aber nur sieben Handgriffe.** 4.3 fällt raus, 5.34 kommt
dazu. Und 6.5 und 6.1 tragen **keinen eigenen Handgriff** — sie entblocken sich
mit 6.6. Wer die Liste als elf Aufgaben liest, sucht vier, die es nicht gibt. →
`03ea08e`

**④ Die Phasengewichte: Phase 10 ist zu niedrig angesetzt.** Gemessen: `bot.py`
hat **12.628 Zeilen**, alle eigenen Module zusammen **19.002**. In Phase 10 liegen
der Aufräumpass über diesen Bestand, die Modularisierung, die Nachmessung aller
104 Punkte, 25 fehlende Drehbuch-Einträge und 18 Punkte ohne Akzeptanzkriterium.
**Drei Prozent trägt das nicht.** Die Folge ist unbequem: Phase 10 steht bei 0 %,
ein höheres Gewicht **senkt** den Gesamtstand. *Die 57 % sind damit eher zu
freundlich als zu streng.* — Kein Eingriff, das ist Engywucks Urteil und seine
Entscheidung. (Die Zahl in `CLAUDE.md`, die sein Gewicht gestützt hätte, stand
auf *über 7000* und war seit dem 25.07. nicht nachgezogen → `b3ae9a4`.)

## Wo ich gesucht und nichts gefunden habe

*Eine Gegenprüfung, die nie etwas findet, ist selbst der Befund — aber nur, wenn
sichtbar ist, wo sie hingesehen hat.*

- **Ein zweites Raster für die namenlosen Bausteine**: alle 21 Wurzelmodule und
  alle Betriebsskripte, erst gegen den Dateinamen, dann gegen den Klarnamen.
  Zwischenergebnis waren 22 Treffer — nach der Klarnamen-Probe blieben **exakt
  seine beiden übrig** (`wachmuster.py`, `mailtext.py`); die übrigen stehen unter
  ihrem Klarnamen im Drehbuch, teils sehr präsent (*Stundenblume* 10×,
  *Kontingent* 27×). **Mein Raster war zu grob, seines trägt.**
- **Die sechs Falsch-Positive**, einzeln am Blocktext statt am Suchmuster.
- **Ob 4.3 früher einmal die Ordnerspiegelung war** (`git log -S` über die
  Überschrift) — war es nie.

## Eine Unstimmigkeit, kein Befund

Die Übersicht spricht von **sieben** Falsch-Positiven und nennt **sechs** Nummern
(3.2, 5.19, 9.2, 9.6, 9.11, 10.1). Vermutlich ist 5.15 die siebte — sie steht in
der fünften Methode neben 5.19. **Nicht abgeleitet**, nur vermerkt.
