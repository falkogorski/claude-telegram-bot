...
- **🏗 Umsetzungsplan (17.07., aus Architektur-Analyse):** Persistenz im **einen Trichter `process_user_text`** (alle 5 Nachrichtentypen). `QueuedJob.update` (lebendes `telegram.Update`) ist **nicht serialisierbar** → gespeichert werden nur die **Primitive** (user_id, chat_id, message_id, thread_id, text, Anhang-Dateipfade aus `UPLOAD_DIR`, force_tts/output_chat_id/reply_to/thorough) + `status` (offen/in_bearbeitung/beantwortet) + Zeitstempel (`time.time()`, NICHT monotonic — überlebt Reboot). **Format:** atomare Per-Nachricht-Datei `logs/pending/<key>.json` (tmp+`os.replace`); beim Beantworten löschen, „offen" = liegt noch da. Neues Modul `pending.py` (analog `ampel.py`/`presend.py`). Status-Übergänge im Worker (`_session_worker`/`_run_job`). **Startup-Reconcile** in `post_init`. Wiederaufgreif-Pfad arbeitet auf den Primitiven, NICHT auf `update.*`. Reihenfolge: erst diese Schicht, dann 5.18. **Neustart-Verhalten: Adam-Entscheid offen** (auto nachbearbeiten / nur melden / hybrid).
- **Adam-Bestätigung:** —
- **Verifiziert am:** —
